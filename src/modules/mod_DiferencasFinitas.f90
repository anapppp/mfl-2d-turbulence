...
! usando Gauss-Seidel e SOR. 
! Condição de Contorno: Dirichlet no topo 
!                       df/dy = 0.0  (imposicao da pressao)
!                       ciclica nas laterais

real :: Ax, Ay, B, omega, erromax, errolim, erro, ptoFantasma
real :: dx(:), dy(:), g(:,:), f(:,:), fold(in,jn)
integer :: i, j, in, jn, cont, ierrmax, jerrmax
  ...
  do i=2,in-1
    ...
    do j=1,jn-1     
      ...
      if (j==1) then
        Ay = 2.0/dy(j)/dy(j)/(dy(j)+dy(j))
        B = 2.0 * ( 1.0/dx(i)/dx(i-1) + 1.0/dy(j)/dy(j) )
        ptoFantasma = dy(j)*fold(i,j+1) + (dy(j)-dy(j))*fold(i,j) 
        f(i,j) = Ax/B * ( dx(i)*f(i-1,j) + dx(i-1)*fold(i+1,j) ) + &
                 Ay/B * ( ptoFantasma + dy(j)*fold(i,j+1) ) - &
                 g(i,j)/B
      else
        Ay = 2.0/dy(j)/dy(j-1)/(dy(j)+dy(j-1))
        B = 2.0 * ( 1.0/dx(i)/dx(i-1) + 1.0/dy(j)/dy(j-1) )
...
! Condicao de Contorno Ciclica: calculo f em i=in

  Ax = 2.0/dx(1)/dx(in-1)/(dx(1)+dx(in-1))
     
  do j=1,jn-1     

    if (j==1) then
      Ay = 2.0/dy(j)/dy(j)/(dy(j)+dy(j))
      B = 2.0 * ( 1.0/dx(1)/dx(in-1) + 1.0/dy(j)/dy(j) )
      ptoFantasma = dy(j)*fold(in,j+1) + (dy(j)-dy(j))*fold(in,j) 
      f(in,j) = Ax/B * ( dx(1)*f(in-1,j) + dx(in-1)*fold(2,j) ) + &
                Ay/B * ( ptoFantasma + dy(j)*fold(in,j+1) ) - &
                g(in,j)/B
    else
      Ay = 2.0/dy(j)/dy(j-1)/(dy(j)+dy(j-1))
      B = 2.0 * ( 1.0/dx(1)/dx(in-1) + 1.0/dy(j)/dy(j-1) )
      f(i,j) = Ax/B * ( dx(1)*f(in-1,j) + dx(in-1)*fold(2,j) ) + &
               Ay/B * ( dy(j)*f(in,j-1) + dy(j-1)*fold(in,j+1) ) - &
               g(in,j)/B
    endif
      
    f(in,j) = omega*f(in,j) + (1-omega)*fold(in,j)
